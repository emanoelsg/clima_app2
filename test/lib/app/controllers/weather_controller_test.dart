...
void main() {
  ...
  setUp(() {
    ...
    when(mockService.fetchWeather(city: anyNamed('city'))).thenAnswer(
    ...
    when(mockService.getHourlyForecast(city: anyNamed('city'))).thenAnswer(
      (_) async => [
        HourlyForecast(time: '12:00', iconCode: '01d', temp: 25),
      ],
    );

    when(mockService.getWeeklyForecast(city: anyNamed('city'))).thenAnswer(
      (_) async => WeeklyForecast(daily: []),
    );
  ...
  test('loadAll com erro retorna mensagem de erro', () async {
    when(mockService.fetchWeather(city: anyNamed('city')))
